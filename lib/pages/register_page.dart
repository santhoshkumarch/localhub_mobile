...
class _RegisterPageState extends State<RegisterPage> {
  ...
  void _checkUser() async {
    try {
      final userCheck = await ApiService.checkUser(widget.phone).timeout(const Duration(seconds: 10));
      setState(() {
        _userExists = userCheck['exists'] ?? false;
        _checking = false;
      });
    } catch (e) {
      setState(() {
  ...
  void _register() async {
    ...
    try {
      ...
      if (userCheck['exists'] == true) {
        final otpSent = await ApiService.sendOtp(widget.phone).timeout(const Duration(seconds: 10));
        if (otpSent) {
        ...
        final success = await ApiService.registerUser(widget.phone, name).timeout(const Duration(seconds: 10));
        if (success) {
          final otpSent = await ApiService.sendOtp(widget.phone).timeout(const Duration(seconds: 10));
          if (otpSent) {
            await AuthService.saveLastPhone(widget.phone);
  ...
  Future<Map<String, dynamic>> _checkUserExists(String phone) async {
    try {
      final result = await ApiService.checkUser(phone);
      return result;
    } catch (e) {
      return {'exists': false};
    }
                            ...
                            decoration: InputDecoration(
                              ...
                              focusedBorder: OutlineInputBorder(
                                ...
                                borderSide: const BorderSide(
                                    color: Color(0xFFDC143C), width: 2),
                              ),
                            ),
                          ),
                        if (!_userExists && !_checking)
